...
static void profile_delete(profile_t* profile) {
    if (profile) {
        ...
        // add samplename
        if (profile->samplename) {
            g_free(profile->samplename);
        }
...
// add samplename
static void profile_set_sample(profile_t* profile, const gchar* s) {
    if (profile) {
        if (profile->samplename)
            g_free(profile->samplename);
        profile->samplename = g_strdup(s);
    } else g_print("profile_set_sample(): profile==NULL\n");
}

static gchar* profile_get_sample(profile_t* profile) {
    if (profile) {
        return profile->samplename;
    } else {
        g_print("profile_get_sample(): profile==NULL\n");
        return NULL;
    }
}
...
static void selection_changed_cb(metro_t* metro)
{
    ...
    if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
        ...
        // add samplename: synch profil -> metro
        set_sample(metro, NULL, profile_get_sample(profile));
    } else if (debug)
                ...
                if (n >= 0) {
                    ...
                    if (tmp) {
                                 ...
                                 gtk_tree_view_get_model(metro->profiles_tree))))
                        {
                            ...
                            // add samplename
                            } else if (!strcmp(tmp, "SampleFilename")) {
                                profile_set_sample(profile, option_value);
                            } else {
...
static int get_n_profile(metro_t* metro)
{
         ...
         GTK_LIST_STORE(gtk_tree_view_get_model(metro->profiles_tree))))
    {
        return gtk_tree_model_iter_n_children(GTK_TREE_MODEL(store), NULL)
                * nb_options /* name + profile data entries */;
    } else {
                               ...
                               int n, char** option_name)
{
    ...
    int profile_n = n / nb_options;
    int suboption_n = n % nb_options;
         ...
         GTK_LIST_STORE(gtk_tree_view_get_model(metro->profiles_tree))))
    {
                    ...
                    GTK_TREE_MODEL(store), &iter, NULL, profile_n))
        {
            ...
            case 3:
                ...
                // add samplename
            case 4:
                name_suffix = "SampleFilename";
                value = profile_get_sample(profile);
                break;
            default:
...
static void save_profile(metro_t* metro, GtkTreeIter* iter)
{
    ...
    // add samplename
    profile_set_sample(profile, get_sample(metro, 1, NULL));
}
